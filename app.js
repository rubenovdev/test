...
;(function () {
  ...
  function animateNumber(el) {
    ...
    // Число может идти с разделителями тысяч («458», «22 354»), но пробел
    // после него принадлежит тексту: иначе «до 6 месяцев» станет «до6месяцев».
    ...
    var duration = 900
    // Номер в реестре – идентификатор, а не сумма: разделители тысяч его портят
    var isNumberId = /№/.test(prefix)

    function format(value) {
      return isNumberId ? String(value) : value.toLocaleString('ru-RU')
    }
  ...
  // Две ветки разбора: судебная и бесплатный МФЦ. Первый вопрос общий,
  // дальше сценарий выбирается по ответу про сумму долга.
  var CHAT_SCRIPTS = {
    court: [
      FIRST_QUESTION,
      {
    ...
    mfc: [
      FIRST_QUESTION,
      {
        ai: 'Тогда сначала проверим бесплатный путь: до 1 млн ₽ работает внесудебное банкротство через МФЦ, и платить за него не нужно никому. Ключевой вопрос – что у вас с приставами: производство окончено и исполнительный лист вернули взыскателю, или взыскание еще идет?',
        quick: ['Производство окончено, лист вернули', 'Приставы еще работают']
      },
      {
        ai: 'Понял. И еще одно: есть ли имущество кроме единственного жилья – машина, доля в другой квартире, дача? От этого зависит, пройдет ли внесудебная процедура.',
        quick: ['Ничего нет', 'Есть машина']
      },
      {
        ai: '<b>Похоже, вам подходит бесплатное банкротство через МФЦ.</b> Долг до 1 млн ₽, производство окончено, имущества нет – это условия внесудебной процедуры. Она длится шесть месяцев, управляющего в ней нет, и наши услуги вам не нужны. Важная деталь: спишутся только те долги, которые вы сами укажете в заявлении, поэтому список нужно собрать полностью.',
        result: 'mfc'
      }
    ]
  }
  ...
  function initChat() {
    ...
    function resultCard(kind) {
      ...
      if (kind === 'mfc') {
        el.innerHTML =
          '<div><span class="tag tag--free">Вам не нужно платить</span>' +
          '<h3 style="margin-top:12px">Внесудебное банкротство через МФЦ</h3></div>' +
          '<div class="result-rows">' +
          '<div><span>Сумма долга</span><span>до 1 млн ₽ – лимит соблюден</span></div>' +
          '<div><span>Обязательные расходы</span><span>0 ₽: ни депозита, ни пошлины</span></div>' +
          '<div><span>Наша работа</span><span>не требуется</span></div>' +
          '<div><span>Срок процедуры</span><span>6 месяцев с даты публикации</span></div>' +
          '<div><span>Главный риск</span><span>забыть кредитора в заявлении</span></div>' +
          '</div>' +
          '<p class="tiny muted">Проверка предварительная: основания внесудебной процедуры юрист подтвердит по вашим документам. Если что-то не сойдется в МФЦ – напишите, разберемся.</p>'
      } else {
        el.innerHTML =
    ...
    function captureCard(kind) {
      var el = document.createElement('div')
      el.className = 'capture bubble--enter'
      el.innerHTML =
        (kind === 'mfc'
          ? '<h3>Куда отправить инструкцию по МФЦ</h3>' +
            '<p class="muted small">Пришлем порядок подачи, форму заявления и список того, что важно указать, чтобы не потерять долги. Это бесплатно – мы на вашем случае не зарабатываем.</p>'
    ...
    function lawyerReply(kind) {
      ...
      var answer =
        kind === 'mfc'
          ? '<b>Дмитрий Ерзиков, финансовый управляющий:</b> подтверждаю разбор: ваш случай проходит через МФЦ, платить за процедуру не нужно – ни нам, ни суду. Соберите справки по всем кредиторам, включая микрозаймы и коммунальные: спишется только то, что попадет в заявление. Инструкцию отправил, если в МФЦ что-то не примут – напишите сюда.'
    ...
    var FALLBACK = {
      ...
      кредитор:
        'Во внесудебной процедуре списываются только те долги, которые указаны в заявлении: забытый кредитор останется с вами и после списания. Поэтому список собираем по справкам, а не по памяти.'
    }
    ...
    // Ветка разбора выбирается по первому ответу про сумму долга:
    // до 1 млн ₽ – проверяем бесплатный МФЦ, больше – судебная процедура.
    function detectBranch(text) {
      var lower = text.toLowerCase().replace(',', '.')

      if (/меньше миллион|до миллион|менее миллион/.test(lower)) return 'mfc'

      var millions = lower.match(/(\d+(?:\.\d+)?)\s*млн/)
      if (millions) return Number(millions[1]) < 1 ? 'mfc' : 'court'

      var thousands = lower.match(/(\d+(?:\.\d+)?)\s*(?:тыс|тысяч)/)
      if (thousands) return Number(thousands[1]) < 1000 ? 'mfc' : 'court'

      var plain = lower.match(/(\d[\d\s]{4,})/)
      if (plain) {
        return Number(plain[1].replace(/\s/g, '')) < 1000000 ? 'mfc' : 'court'
      }
    ...
    function send(text) {
      ...
      if (step === 1) script = CHAT_SCRIPTS[detectBranch(text)]

      if (step < script.length) {
        var node = script[step]
        step += 1
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            if (node.result) {
              setTimeout(function () {
                resultCard(node.result)
                captureCard(node.result)
                busy = false
              }, 700)
            } else {
    ...
    function start() {
      ...
      setTimeout(function () {
        typing.remove()
        addBubble('ai', FIRST_QUESTION.ai)
        renderQuick(FIRST_QUESTION.quick)
        step = 1
        busy = false
      }, 900)
    }
